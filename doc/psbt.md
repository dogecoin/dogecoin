# PSBT Howto for Dogecoin Core

Since Dogecoin Core 0.17, an RPC interface exists for Partially Signed Dogecoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/dogecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Dogecoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://dogecointalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Dogecoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Dogecoin transaction (in network format)
...
#### Multisig with multiple Dogecoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Dogecoin Core. We assume their wallets only contain the multisig funds. In case
