...
`indexes/txindex/` | LevelDB database      | Optional transaction index database
`indexes/blockfilter/<type>/` | LevelDB database | Optional block filter index database, one per filter type; created by `-blockfilterindex`
`indexes/blockfilter/<type>/` | `fltr?????.dat`  | Optional block filter data, stored alongside the index database
