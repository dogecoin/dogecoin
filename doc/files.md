...
The data directory is the default location where the Pepecoin Core files are stored.
...
Windows  | `%APPDATA%\Pepecoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `pepecoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `pepecoin.conf`       | User-defined configuration settings for `pepecoind` or `pepecoin-qt`; can be specified by `-conf` option
`./`               | `pepecoind.pid`        | Stores the process ID (PID) of `pepecoind` or `pepecoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `pepecoind` or `pepecoin-qt`
...
`pepecoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Pepecoin Core:
