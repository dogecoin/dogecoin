...
Clone the git repositories for dogecoin and gitian and then checkout the dogecoin version that you want to build.
...
To build Dogecoin (for Linux, OSX and Windows) just follow the steps under 'perform
...
./bin/gbuild --commit bitcoin=${COMMIT} --url dogecoin=${URL} ../dogecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dogecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dogecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/dogecoin-build.assert
```
