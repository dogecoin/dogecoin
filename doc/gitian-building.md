...
*Setup instructions for a Gitian build of Garudacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Garudacoin
...
- [Building Garudacoin Core](#building-garudacoin-core)
...
Clone the git repositories for garudacoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/garudacoin/garudacoin.git
git clone https://github.com/garudacoin/gitian.sigs.git
```
...
in the garudacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Garudacoin Core
----------------

To build Garudacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the garudacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/garudacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/garudacoin/garudacoin
...
For example:
```bash
URL=https://github.com/laanwj/garudacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit garudacoin=${COMMIT} --url garudacoin=${URL} ../garudacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit garudacoin=${COMMIT} --url garudacoin=${URL} ../garudacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit garudacoin=${COMMIT} --url garudacoin=${URL} ../garudacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the garudacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../garudacoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/garudacoin/garudacoin-detached-sigs.git

BTCPATH=/some/root/path/garudacoin
SIGPATH=/some/root/path/garudacoin-detached-sigs

./bin/gbuild --url garudacoin=${BTCPATH},signature=${SIGPATH} ../garudacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/garudacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/garudacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/garudacoin-osx-build.assert
```
...
[garudacoin/gitian.sigs](https://github.com/garudacoin/gitian.sigs/) repository, or if that's not possible create a pull
