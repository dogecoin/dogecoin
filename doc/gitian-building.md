...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of ScrapCoin Core repository.
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/scrapcoin-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/scrapcoin-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/scrapcoin-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/scrapcoin/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [scrapcoin/gitian.sigs](https://github.com/scrapcoin/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of ScrapCoin Core*!**
