ScrapCoin Fee Recommendation
----------------------------

_last updated for 1.14.6_

The ScrapCoin chain has a relatively low block interval, 1 megabyte blockspace
and aims to provide a cheap means for people to transact. Therefore, the biggest
threat to the ScrapCoin chain as a whole is spam and in 2014, a transaction fee
and dust disincentive were introduced, to combat on-chain spam.

ScrapCoin Core implements a number of defaults into the software that reflect the
...
parametrized. The recommended defaults, as implemented in the ScrapCoin Core
wallet, are:

- **0.01 SCRAP per kilobyte** transaction fee
- **0.01 SCRAP** dust limit (discard threshold)
- **0.001 SCRAP** replace-by-fee increments

The wallet rejects transactions that have outputs under the dust limit, and
discards change to fee if it falls under this limit.

Note: In the past, ScrapCoin has enforced a rounding function in the fee
      ...
      transaction only has to pay `0.01 / 1000 * 192 = 0.00192` SCRAP fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 SCRAP/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 SCRAP/kB**,
...
- The hard dust limit is set at **0.001 SCRAP** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 SCRAP** - sending a transaction with outputs
  under this value, are required to add 0.01 SCRAP for each such output, or else
