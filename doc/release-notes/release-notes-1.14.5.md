ScrapCoin Core version 1.14.5 is now available from:

  <https://github.com/scrapcoin/scrapcoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All ScrapCoin Core users, miners, services, relay
...
To receive security and update notifications, please watch reddit or Twitter:
  ...
  * @ScrapCoin on Twitter for high priority announcements
  * @scrapcoin\_devs on Twitter for updates on development work
...
most ScrapCoin Core users.

### Remote Code Execution in ScrapCoin QT (CVE-2021-3401)
...
**ScrapCoin QT users are urged to please update their installations to this
...
leakage of sensitive information when ScrapCoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**ScrapCoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the ScrapCoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 SCRAP/kb, and
* the recommended dust limit is 1 SCRAP, and
* the recommended RBF increment is 0.001 SCRAP.

### Wallet/UI Changes

* The user interface for selecting fees when transacting SCRAP has been updated
  to give an idea of how much is being spent, rather than a block target. As
  ScrapCoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for ScrapCoin.
...
* The default minimum transaction fee is now 0.01 SCRAP per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 SCRAP, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 SCRAP.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 SCRAP and sets the value under
...
The Berkley DB version used by ScrapCoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within ScrapCoin Core.
