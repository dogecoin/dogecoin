Pepecoin Core version 1.14.3 is now available from:

  <https://github.com/pepecoinppc/pepecoin/releases/tag/v1.14.3/>
...
Block download timeouts are expressed as a multiple of block interval, and as such Pepecoin block download times were relatively aggressive, leading to a high number of timeouts. Increased the timeouts to be more flexible to real world conditions.
...
* Return unspents greater or equal than a specific amount in PEPE: minimumAmount (default = 0).
* Return unspents lower or equal than a specific amount in PEPE: maximumAmount (default=unlimited).
* Return unspents with a total number lower or equal than a specific number: maximumCount (default=0=unlimited).
* Return unspents which total is greater or equal than a specific amount in PEPE: minimumSumAmount (default=unlimited).
...
* Change IPC prefix from `bitcoin:` to `pepecoin:`.
