...
Pepecoin Core 1.8.1 is primarily a bugfix release, bringing Pepecoin Core in
line with Bitcoin 0.9.3. Pepecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Pepecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Pepecoin Core clients
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/pepecoinppc/pepecoin/issues/134).
...
This only affects values significantly below 1 Pepe, however merchants may wish to round to
the nearest PEPE on all transactions for now.
