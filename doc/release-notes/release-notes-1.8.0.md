...
Garudacoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Garudacoin block chain. Garudacoin Core 1.8 also enables payment protocol
support for Garudacoin (note that Garudacoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Garudacoin.
Users remaining on the previous client **will not** be able to receive or spend
Garudacoin (with the exception of with other nodes who have not upgraded).
...
use "garudacoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Garudacoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Garudacoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Garudacoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Garudacoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/garudacoin/dips/ .
The key differencs between the Bitcoin and Garudacoin payment protocols are that
the Garudacoin payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 Garuda, or around $0.00012 per transaction).
...
of Garudacoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled garudacoin: URIs under Windows has been resolved, and as
part of this work network detection for garudacoin: URIs is now more robust.
