...
import os
import sys
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "python-dogecoinrpc"))
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    dogecoind and bitcoin-cli must be in search path.
    """
...
def start_nodes(num_nodes, dir):
    # Start dogecoinds, and wait for RPC interface to be up and running:
    devnull = open("/dev/null", "w+")
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "dogecoind", "-datadir="+datadir ]
        dogecoind_processes.append(subprocess.Popen(args))
