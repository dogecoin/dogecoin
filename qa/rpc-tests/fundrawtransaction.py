...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']}]
        # Dingocoin: Fee is exact, do not use tolerance
        ...
        assert_raises_jsonrpc(-5, "changeAddress must be a valid dingocoin address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']} ]
        # Dingocoin: Reduce this output so the fee doesn't leave us with no change
