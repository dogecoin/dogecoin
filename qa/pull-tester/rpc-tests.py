...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_SCRAPCOIND' not in vars():
    ENABLE_SCRAPCOIND=0
...
#Set env vars
if "SCRAPCOIND" not in os.environ:
    os.environ["SCRAPCOIND"] = BUILDDIR + '/src/scrapcoind' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_SCRAPCOIND == 1):
    print("No rpc tests to run. Wallet, utils, and scrapcoind must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie scrapcoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `scrapcoin-cli help` (`rpc_interface.txt`).
