...
\tikzset{
  decision/.style = {diamond, draw, fill=blue!20, text width=4.5em, text badly centered, node distance=3.5cm, inner sep=0pt},
...
\node [block, left of=threat, node distance=4.5cm] (ecdsa) {Generate ECDSA Keypair};
\node [block, right of=threat, node distance=4.5cm] (dilithium) {Generate Dilithium Keypair};
...
\path [line] (threat) -| node [near start] {No} (ecdsa.east);
\path [line] (threat) -| node [near start] {Yes} (dilithium.west);
...
\begin{table}[H]
\centering
\caption{Consolidated PAT Performance Metrics (n=10,000 signatures)}
\label{tab:consolidated_benchmarks}
\resizebox{\textwidth}{!}{
\begin{tabular}{@{}lccccc@{}}
\toprule
Strategy & Compression Ratio & Throughput (sigs/sec) & Memory (MB) & Energy (kWh) & CO2e (kg) \\
\midrule
Logarithmic & 34,597x & 96 ± 12 & 0.0 & 1.56e-10 & 0.0 \\
Threshold & 12,845x & 89 ± 8 & 0.2 & 1.72e-10 & 0.0 \\
Merkle Batch & 8,923x & 91 ± 11 & 0.1 & 1.68e-10 & 0.0 \\
Stacked Multi & 1.2x & 156 ± 18 & 0.0 & 9.81e-11 & 0.0 \\
\bottomrule
\end{tabular}
}
\end{table}
...
Complete implementation available at: \href{https://github.com/odenrider/dogecoin/tree/pat-aggregation-prototype/src/pat}{GitHub Repository (PAT branch)}
