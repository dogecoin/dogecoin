...
const char * const BITCOIN_CONF_FILENAME = "scrapcoin.conf";
const char * const BITCOIN_PID_FILENAME = "scrapcoind.pid";
...
#else
    const char* pszModule = "scrapcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ScrapCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ScrapCoin";
#else
    // Unix
    return pathRet / ".scrapcoin";
#endif
#endif
}
