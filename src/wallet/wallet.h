...
/* 1.14.5: set the wallet's discard threshold to 1 SCRAP because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 SCRAP. Very network
...
/*
 * ScrapCoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * ScrapCoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * ScrapCoin: Get a fee targeting a specific transaction speed.
     */
    CAmount GetScrapCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * ScrapCoin: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetScrapCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the ScrapCoin dust fee.
     */
