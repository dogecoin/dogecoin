...
//MLUMIN: 10/2021 Add these functions back from 1.14 regression, attempt
    ...
    if (pindexPrev != chainActive.Tip()
            ...
            && GetTime() - nStart > 60))
        {
            ...
            std::unique_ptr<CBlockTemplate> newBlock
                = BlockAssembler(Params()).CreateNewBlock(scriptPubKey,true);
            if (!newBlock)
                throw JSONRPCError(RPC_OUT_OF_MEMORY, "out of memory");
...
UniValue getauxblockbip22(const JSONRPCRequest& request)
{
    ...
    //MLUMIN: 10/2021 - Call AuxMiningCheck(); instead.
    AuxMiningCheck();
    /*
    if(!g_connman)
        throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
    ...
    */
    //MLUMIN: 10/2021 - Section end

    // The variables below are used to keep track of created and not yet
    // submitted auxpow blocks.  Lock them to be sure even for multiple
    // RPC threads running in parallel.  

    //MLUMIN: 10/2021 - These have been moved above create/submit/checkaux.
    /*
    static CCriticalSection cs_auxblockCache;
    LOCK(cs_auxblockCache);
    static std::map<uint256, CBlock*> mapNewBlock;
    ...
    */
    ...
    //MLUMIN 10/2021: Note, this should return AuxMiningCreateBlock(coinbaseScript->ReverseScript);
    {
        static unsigned nTransactionsUpdatedLast;
        static const CBlockIndex* pindexPrev = nullptr;
        static uint64_t nStart;
...
//MLUMIN: 10/2021 - test on replacing createauxblock
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    //MLUMIN: 10/2021 add back in create/submit
