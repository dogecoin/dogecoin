...
#if defined(NDEBUG)
# error "Garudacoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Garudacoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetGarudacoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Garudacoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("garudacoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Garudacoin: BIP30 has been active since inception
    ...
    // Garudacoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetGarudacoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Garudacoin: Disallow AuxPow blocks before it is activated.
    ...
    // Garudacoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Garudacoin: We probably want to disable this
    int nLockTimeFlags = 0;
