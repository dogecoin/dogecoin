...
#if defined(NDEBUG)
# error "ScrapCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "ScrapCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetScrapCoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // ScrapCoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("scrapcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // ScrapCoin: BIP30 has been active since inception
    ...
    // ScrapCoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetScrapCoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // ScrapCoin: Disallow AuxPow blocks before it is activated.
    ...
    // ScrapCoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // ScrapCoin: We probably want to disable this
    int nLockTimeFlags = 0;
