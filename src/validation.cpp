...
namespace {
    ...
    struct CBlockIndexWoofComparator
    {
        bool operator()(CBlockIndex *pa, CBlockIndex *pb) const {
            // First sort by most total woof, ...
            if (pa->nChainWoof > pb->nChainWoof) return false;
            if (pa->nChainWoof < pb->nChainWoof) return true;
     ...
     */
    std::set<CBlockIndex*, CBlockIndexWoofComparator> setBlockIndexCandidates;
    ...
    /** chainWoof for the last block that preciousblock has been applied to. */
    arith_uint256 nLastPreciousChainWoof = 0;
...
bool AcceptToMemoryPoolWoofer(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
...

    {
        ...
        if (fReplacementTransaction)
        {
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++)
            {
                ...
                // but for now requiring all new inputs to be confirmed Woofs.
                        ...
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee)
{
    ...
    bool res = AcceptToMemoryPoolWoofer(pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, vHashTxToUncache);
    if (!res) {
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    // Check the header
    if (fCheckPOW && !CheckAuxPowProofOfWoof(block, consensusParams))
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip() == NULL)
        return true;
    if (chainActive.Tip()->nChainWoof < UintToArith256(chainParams.GetConsensus(chainActive.Height()).nMinimumChainWoof))
        return true;
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWoof > chainActive.Tip()->nChainWoof + (GetBlockProof(*chainActive.Tip()) * 30)))
    {
        if (!GetfLargeWoofForkFound() && pindexBestForkBase)
        {
            std::string warning = std::string("'Warning: Large-woof fork detected, forking after block ") +
        ...
        if (pindexBestForkTip && pindexBestForkBase)
        {
            ...
            SetfLargeWoofForkFound(true);
        }
        else
        {
            LogPrintf("%s: Warning: Found invalid chain at least ~6 blocks longer than our best chain.\nChain state database corruption likely.\n", __func__);
            SetfLargeWoofInvalidChainFound(true);
        }
    }
    else
    {
        SetfLargeWoofForkFound(false);
        SetfLargeWoofInvalidChainFound(false);
    }
}
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || (pindexBestForkTip && pindexNewForkTip->nHeight > pindexBestForkTip->nHeight)) &&
            pindexNewForkTip->nChainWoof - pfork->nChainWoof > (GetBlockProof(*pfork) * 7) &&
...
{
    if (!pindexBestInvalid || pindexNew->nChainWoof > pindexBestInvalid->nChainWoof)
        pindexBestInvalid = pindexNew;

    LogPrintf("%s: invalid block=%s  height=%d  log2_woof=%.8g  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->nChainWoof.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    CBlockIndex *tip = chainActive.Tip();
    assert (tip);
    LogPrintf("%s:  current best=%s  height=%d  log2_woof=%.8g  date=%s\n", __func__,
      tip->GetBlockHash().ToString(), chainActive.Height(), log(tip->nChainWoof.getdouble())/log(2.0),
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != mapBlockIndex.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex &&
                ...
                pindexBestHeader->nChainWoof >= UintToArith256(consensus.nMinimumChainWoof)) {
                ...
                // The test against nMinimumChainWoof prevents the skipping when denied access to any chain at
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_woof=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utx)", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->nVersion,
      log(chainActive.Tip()->nChainWoof.getdouble())/log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
...
/**
 * Return the tip of the chain with the most woof in it, that isn't
 * known to be invalid (it's however far from certain to be valid).
 */
static CBlockIndex* FindMostWoofChain() {
    do {
        CBlockIndex *pindexNew = NULL;

        // Find the best candidate header.
        {
            std::set<CBlockIndex*, CBlockIndexWoofComparator>::reverse_iterator it = setBlockIndexCandidates.rbegin();
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // for the most woof chain if we come across them; we can't switch
            // to a chain unless we have all the non-active-chain parent blocks.
            bool fFailedChain = pindexTest->nStatus & BLOCK_FAILED_MASK;
            bool fMissingData = !(pindexTest->nStatus & BLOCK_HAVE_DATA);
            if (fFailedChain || fMissingData) {
                // Candidate chain is not usable (either invalid or missing data)
                if (fFailedChain && (pindexBestInvalid == NULL || pindexNew->nChainWoof > pindexBestInvalid->nChainWoof))
                    pindexBestInvalid = pindexNew;
...
static void PruneBlockIndexCandidates() {
    ...
    std::set<CBlockIndex*, CBlockIndexWoofComparator>::iterator it = setBlockIndexCandidates.begin();
...
/**
 * Try to make some progress towards making pindexMostWoof the active block.
 * pblock is either NULL or a pointer to a CBlock corresponding to pindexMostWoof.
 */
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWoof, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    AssertLockHeld(cs_main);
    const CBlockIndex *pindexOldTip = chainActive.Tip();
    const CBlockIndex *pindexFork = chainActive.FindFork(pindexMostWoof);
    ...
    while (fContinue && nHeight != pindexMostWoof->nHeight) {
        // Don't iterate the entire list of potential improvements toward the best tip, as we likely only need
        // a few blocks along the way.
        int nTargetHeight = std::min(nHeight + 32, pindexMostWoof->nHeight);
        vpindexToConnect.clear();
        vpindexToConnect.reserve(nTargetHeight - nHeight);
        CBlockIndex *pindexIter = pindexMostWoof->GetAncestor(nTargetHeight);
        ...
        BOOST_REVERSE_FOREACH(CBlockIndex *pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWoof ? pblock : std::shared_ptr<const CBlock>(), connectTrace)) {
                if (state.IsInvalid()) {
                ...
                if (!pindexOldTip || chainActive.Tip()->nChainWoof > pindexOldTip->nChainWoof) {
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    CBlockIndex *pindexMostWoof = NULL;
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            if (pindexMostWoof == NULL) {
                pindexMostWoof = FindMostWoofChain();
            }

            // Whether we have anything to do at all.
            if (pindexMostWoof == NULL || pindexMostWoof == chainActive.Tip())
                return true;

            bool fInvalidFound = false;
            std::shared_ptr<const CBlock> nullBlockPtr;
            if (!ActivateBestChainStep(state, chainparams, pindexMostWoof, pblock && pblock->GetHash() == pindexMostWoof->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))
                return false;
    ...
    } while (pindexNewTip != pindexMostWoof);
...
bool PreciousBlock(CValidationState& state, const CChainParams& params, CBlockIndex *pindex)
{
    {
        LOCK(cs_main);
        if (pindex->nChainWoof < chainActive.Tip()->nChainWoof) {
            // Nothing to do, this block is not at the tip.
            return true;
        }
        if (chainActive.Tip()->nChainWoof > nLastPreciousChainWoof) {
            // The chain has been extended since the last call, reset the counter.
            nBlockReverseSequenceId = -1;
        }
        nLastPreciousChainWoof = chainActive.Tip()->nChainWoof;
        setBlockIndexCandidates.erase(pindex);
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->nChainWoof = (pindexNew->pprev ? pindexNew->pprev->nChainWoof : 0) + GetBlockProof(*pindexNew);
    pindexNew->RaiseValidity(BLOCK_VALID_TREE);
    if (pindexBestHeader == NULL || pindexBestHeader->nChainWoof < pindexNew->nChainWoof)
...
{
    // Check proof of Woof matches claimed amount
    ...
    // (i.e. doesn't check woof limit or whether AuxPoW is enabled)
    if (fCheckPOW && !CheckAuxPowProofOfWoof(block, Params().GetConsensus(0)))
        return state.DoS(50, false, REJECT_INVALID, "high-hash", false, "proof of Woof failed");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check proof of Woof
    if (block.nBits != GetNextWoofRequired(pindexPrev, &block, consensusParams))
        return state.DoS(100, false, REJECT_INVALID, "bad-diffbits", false, "incorrect proof of Woof");
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    {
        CBlockIndex* pindex = item.second;
        pindex->nChainWoof = (pindex->pprev ? pindex->pprev->nChainWoof : 0) + GetBlockProof(*pindex);
        ...
        if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS) && (pindex->nChainTx || pindex->pprev == NULL))
            setBlockIndexCandidates.insert(pindex);
        if (pindex->nStatus & BLOCK_FAILED_MASK && (!pindexBestInvalid || pindex->nChainWoof > pindexBestInvalid->nChainWoof))
            pindexBestInvalid = pindex;
        if (pindex->pprev)
            pindex->BuildSkip();
        if (pindex->IsValid(BLOCK_VALID_TREE) && (pindexBestHeader == NULL || CBlockIndexWoofComparator()(pindexBestHeader, pindex)))
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != NULL) {
        ...
        assert(pindex->pprev == NULL || pindex->nChainWoof >= pindex->pprev->nChainWoof); // For every block except the genesis block, the chainWoof must be larger than the parent's.
        ...
        if (!CBlockIndexWoofComparator()(pindex, chainActive.Tip()) && pindexFirstNeverProcessed == NULL) {
            if (pindexFirstInvalid == NULL) {
        ...
        if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) && pindexFirstNeverProcessed == NULL && pindexFirstMissing != NULL) {
            ...
            if (!CBlockIndexWoofComparator()(pindex, chainActive.Tip()) && setBlockIndexCandidates.count(pindex) == 0) {
                if (pindexFirstInvalid == NULL) {
                    assert(foundInUnlinked);
                }
            }
