...
class AuxpowMiningTest(BitcoinTestFramework):
  ...
  def test_common(self, create, submit):
    ...
    # 340, so that the serialisation of the CScriptNum ends in an extra 01.
    # The vector has length 2, which makes up for 02XX01 as the serialised
    ...
    if not self.options.segwit:
      ...
      assert_equal("02%02x01" % (auxblock['height'] % 256), coinbase[0 : 6])
