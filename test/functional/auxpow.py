...
class AuxPOWTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # 1. mine a non-auxpow block, just to ensure that this node
        # can mine at all, expect: success
        self.nodes[0].generate(1)
        self.sync_all()

        # 2. mine an auxpow block before auxpow is allowed, expect: fail
        try:
            scrypt_auxpow.mineScryptAux(self.nodes[0], "00", True)
        except JSONRPCException as ex:
            if ex.error['message'] == "mining auxblock method is not yet available":
                pass
            else:
                raise ex
        self.sync_all()
        ...
        self.nodes[1].generate(self.DIGISHIELD_START - 1)
        self.sync_all()
